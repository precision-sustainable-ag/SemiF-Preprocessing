...
image_paths = Path("NC_2025-02-21_sample/raws").glob("*.RAW")
...
if True:
    for im_path in image_paths:
        ...
        if(current_ccm is not None):
            ...
            ccm_filename = ntpath.basename(im_path).replace(".RAW", "_ccm.npy")
            np.save(ccm_filename, color_matrix)
            print(color_matrix)
            ...
            cv2.imwrite(im_filename[:-3] + 'png', cv2.cvtColor(colour_image_gamma_adjusted, cv2.COLOR_RGB2BGR),  [cv2.IMWRITE_PNG_COMPRESSION, 0])
...
#        r = RAW2DNG()
#        r.options(t, path="")
#        r.convert(rawImage, filename=im_filename[:-4])
